...
def calculo_fitness(cromosoma, matriz_distancias_tiempo:pd.DataFrame, deposito_idx:int, parametros:pd.DataFrame) -> float:
    ...
    rendimiento_gasolina = parametros.loc[parametros['Parameter'] == 'fuel_efficiency_van_medium_max', 'Value'].values[0]
    costo_fijo_activacion = parametros.loc[parametros['Parameter'] == 'C_fixed', 'Value'].values[0]
    costo_distancia = parametros.loc[parametros['Parameter'] == 'C_dist', 'Value'].values[0]
    costo_tiempo = parametros.loc[parametros['Parameter'] == 'C_time', 'Value'].values[0]
    costo_km = (precio_gasolina / rendimiento_gasolina) + costo_distancia  # Costo por km recorrido
    
    for index, ruta in enumerate(cromosoma):
        if len(ruta) == 0:
            continue
        distancia=ga.distancia_total_ruta(ruta, matriz_distancias_tiempo, deposito_idx)
        tiempo=ga.tiempo_total_ruta(ruta, matriz_distancias_tiempo, deposito_idx)
        costo += costo_fijo_activacion + distancia * costo_km + tiempo * costo_tiempo
    return costo
    ...
    if guardar:
        ...
        for index, ruta in enumerate(mejor_solucion):
            ...
            if distancia_recorrida==0:
                ...
                total_cost = distancia_recorrida * costo_km+ tiempo_total * costo_tiempo + costo_fijo_activacion
            resultados["TotalCost"].append(total_cost)
            
        #guardar dataframe
        data_frame=pd.DataFrame(resultados)
        #export csv
        data_frame.to_csv('metaheuristica/caso_2/verificacion_metaheuristica_GA_A_caso2.csv', sep=',', index=False)
